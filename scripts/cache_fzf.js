...
const cacheDefs = [
  ...
  },
  {
    cmd: "gulp",
    getListOptions:
      "npx gulp --tasks --depth 1 | tail -n +3 | awk '{print $3}' | sort",
  ...
  },
  {
    ...
    getCmdFiles: () =>
      execSync('rg --files --hidden --glob "!.git" 2> /dev/null', {
        encoding: "utf-8",
      })
        .trim()
        .split("\n"),
...
function runCmd(cacheDef, arg) {
  ...
  if (new RegExp(`^${cmd}`, "g").test(arg)) {
    ...
    const newFiles = files.filter((file) => {
      ...
      if (
        !stored[cmd].timestamps[file] ||
        new Date(stored[cmd].timestamps[file]) < fileDate
      ) {
    ...
    if (
      newFiles.length ||
      !files.length ||
      (cacheDef.shouldNotCache && cacheDef.shouldNotCache())
    ) {
